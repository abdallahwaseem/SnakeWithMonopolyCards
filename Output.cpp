...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	///TODO: Calculate the Width & Height of the integer "cardNum" if written using the Current Font (checked)
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height (checked)
	...
	///TODO: Draw the Integer the representing the "cardNum" in the location (x,y) (checked)
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_COIN_SET] = "images\\Menu_Coinset.jpg";
	MenuItemImages[ITM_ADD_CARD] = "images\\Menu_Card.jpg";
	MenuItemImages[ITM_COPY_CARDorCOIN] = "images\\Menu_Copy Card.jpg";
	MenuItemImages[ITM_CUT_CARDorCOIN] = "images\\Menu_Cut Card.jpg";
	MenuItemImages[ITM_PASTE] = "images\\Menu_Paste Card.jpg";
	MenuItemImages[ITM_DELETE_GAME_OBJ] = "images\\Menu_Delete Object.jpg";
	MenuItemImages[ITM_SAVE_GRID] = "images\\Menu_Save Grid.jpg";
	MenuItemImages[ITM_OPEN_GRID] = "images\\Menu_Open Grid.jpg";
	...
	///TODO: Prepare images for each menu item and add it to the list(CHECKED)
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_Dice Value.jpg";
	MenuItemImages[ITM_OFFER_MORTGAGE] = "images\\Menu_Offer_Mortgage.jpg";
	MenuItemImages[ITM_ACCEPT_MORTGAGE] = "images\\Menu_Accept_Mortgage.jpg";
	MenuItemImages[ITM_RETURN_MORTGAGE] = "images\\Menu_Return_Mortgage.jpg";
	MenuItemImages[ITM_SELL_CITY] = "images\\Menu_Sell_City.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\Menu_New Game.jpg";
...
{
	///TODO: Clear what was written on the toolbar using CreatePlayModeToolBar() function of the OUtput class (checked)
	CreatePlayModeToolBar();
	pWind->SetPen(UI.ToolBarColor, 1);
	pWind->SetBrush(UI.ToolBarColor);
	pWind->DrawRectangle(10 * UI.MenuItemWidth, 0, UI.width, UI.ToolBarHeight);
	...
	///TODO: Calculate the Width and Height of the string if drawn using the current font (checked)
	//       (Use GetStringSize() window function) and set the "w" and "h" variables with its width and height (checked) 
	...
	///TODO: Draw the string "info" in the specified location (x, y) (checked)
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	int x1 = cellStartX;
	int y1 = cellStartY;
	int x2 = cellStartX + UI.CellWidth;
	int y2 = cellStartY + UI.CellHeight;
	///TODO: Draw the Cell Rectangle using the appropriate coordinates (checked)
	...
	int w = 0, h = 0;
	pWind->GetIntegerSize(w, h, cellNum);
	///TODO: Get the Width and Height of the Cell Number if written using the current font (checked)
	...
	///TODO: Draw the cell number in the x and y location (checked)
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	///TODO: Validate the playerNum, if not valid return (checked)
	if (playerNum >= 0 && playerNum <= 3 && cellPos.IsValidCell() == true)
	{
		...
		///TODO: Draw the player circle in center(x,y) and filled with the playerColor passed to the function(checked)
...
{
	///TODO: Validate the Cell Position (Must be Vertical Cells AND toCell above fromCell, otherwise, Do NOT draw) (checked)
	...
	if (fromCell.HCell() == toCell.HCell() && fromCell.VCell() > toCell.VCell())
	{
		...
		int endDraw = -toCell.VCell() + fromCell.VCell();
		int startY = fromStartY;
		int cellH = UI.CellHeight;
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ((fromCell.VCell() < toCell.VCell()) && (fromCell.HCell() == toCell.HCell()))
	{
		...
		int x[] = { x12 + xChange, x12, x12 - xChange,x12 };
		int y[] = { y1,y1 + yChange, y1, y1 - yChange };
...
void Output::DrawCoinSet(const CellPosition& cellPos) const
{
	///TODO: Write a complete implementation of this function
	int cellStartX = GetCellStartX(cellPos)+ (UI.CellWidth*0.5);
	int cellStartY = GetCellStartY(cellPos);
	pWind->DrawImage("images\\Menu_Coinset.jpg",cellStartX, cellStartY, UI.CellWidth*0.5, UI.CellHeight*0.5);
	// Note: you will need to use function DrawImage() of the library to draw an image of the coins
}
