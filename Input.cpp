...
int Input::GetInteger(Output *pO) const 
{
	...
	return stoi(Input::GetSrting(pO));
	//DONE
...
{	
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_COPY_CARDorCOIN: return COPY_CARDorCOIN;
			case ITM_CUT_CARDorCOIN: return CUT_CARDorCOIN;
			case ITM_PASTE: return PASTE;
			case ITM_DELETE_GAME_OBJ: return DELETE_GAME_OBJ;
...
CellPosition Input::GetCellClicked() const
{
	...
	else if (UI.InterfaceMode == MODE_PLAY)// Play mode
	{
		cellPos.SetHCell(x / UI.CellWidth);
		cellPos.SetVCell((y - UI.ToolBarHeight) / UI.CellHeight);
	}
