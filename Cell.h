...
public:
	...
	Ladder * HasLadder() const;	    // Checks if pGameObject is a Ladder Then returns it if a ladder or returns NULL if not 
	Snake * HasSnake() const;	    // Checks if pGameObject is a Snake Then returns it if a snake or returns NULL if not 
	CoinSet * HasCoinSet() const;	// Checks if pGameObject is a CoinSet Then returns it if a CoinSet or returns NULL if not 
	Card * HasCard() const;	        // Checks if pGameObject is a Card Then returns it if a card or returns NULL if not 
